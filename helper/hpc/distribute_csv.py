...
# from helper import CONFIG, ROOT_DIR
import helper
from helper.runners import *
...
ALL_QUEUES = json.loads(helper.CONFIG.get('HPC', 'QUEUES'))
...
def generate_microminer_preparation_script():
    ...
    """
    kmer_index = helper.CONFIG['DATABASES']['SITE_SEARCH_DB']
...
def generate_runner_script(runner,):
    """
...
def generate_hpc_script(job_name: str, global_working_dir: Path, local_working_dir: str,
                        input_file: Path, stdout_file: Path, stderr_file: Path,
                        nof_jobs: int, cpus: int, conda_bin_path: Path,
                        conda_env_name: str, queues: list,
...
def distribute_csv(dataset_file: Path, runner, outdir: Path, job_name: str,
                   cpus: int = 1):
    ...
    with tempfile.TemporaryDirectory(dir=helper.CONFIG['HPC']['HPC_WORKING_DIR'],
                                     prefix=tmpdir_name) as t:
        tmpdir = Path(t)
        ...
        job_script_str = generate_hpc_script(job_name=job_name,
                                         global_working_dir=tmpdir,
                                         local_working_dir=helper.CONFIG['HPC'][
                                             'HPC_LOCAL_WORKING_DIR'],
                                         ...
                                         queues=ALL_QUEUES,
                                         conda_bin_path=Path(
                                             helper.CONFIG['HPC']['CONDA_BIN_PATH']),
                                         conda_env_name=helper.CONFIG['HPC']['CONDA_ENV_NAME'],
                                         # copy_ssh=[this_py_module],
                                         add_to_pythonpath=[Path(
                                             helper.CONFIG['HPC']['PYPATH_PATHS'])],
        ...
        cmd_call = ['rsync', '-ra', '--info=progress2', (tmpdir / 'results').resolve(),
                    outdir.resolve()]
        ...
        cmd_call = ['rsync', '-ra', '--info=progress2', (tmpdir / 'cluster_out').resolve(),
                    outdir.resolve()]
        res = subprocess.run(cmd_call, stdout=subprocess.PIPE)
        if res.returncode != 0:
            logger.error(res)
