"""
Runs MicroMiner on a list of input protein structure pairs in pair mode.
"""
...
from helper.runners import MicroMinerPair
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=
        """
        Structure Pair comparison with MicroMiner. 
        """)
    ...
    if is_hpc:
        distribute_csv(dataset_file,
                       runner=MicroMinerPair(cpus=1, raise_error=False),
                       outdir=outdir,
                       job_name='pair',
                       cpus=cpus)
    else:
        runner = MicroMinerPair(cpus=cpus, raise_error=False)
